...
36个预算×上下文×成本格经过high/medium两种映射门禁产生72条记录，其中33条允许同坐标假设换算；越界/不可行者不给分数。输出`q3_bridge_sensitivity.csv`保留源状态、条件Loss、N、假设分数及medium来源族残差范围，实证标定分数和联合95%区间均为空。这满足前问输出进入后问的条件计算，同时避免把A侧目标Loss或半合成质量增益当真实Benchmark提升。
