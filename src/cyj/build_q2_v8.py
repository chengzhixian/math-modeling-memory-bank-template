...
OUT = ROOT / "outputs/Q2"
REPRO_MANIFEST = ROOT / "data/processed/Q2/repro/manifest.json"
N, D = 1.0, 100.0
...
def b9_b10_stress():
    source = json.loads((ROOT/"outputs/Q2/evidence/b9_b10_extrapolation_audit.json").read_text(encoding="utf-8"))
    ...
    output = {"B9_role": "metadata_only", "B10_role": "estimated_loss_stress_only",
              ...
              "source_audit_sha256": sha256(ROOT/"outputs/Q2/evidence/b9_b10_extrapolation_audit.json")}
...
def main():
    ...
    b910 = b9_b10_stress()
    # Historical v7 migration is preserved in upstream evidence, outside the current producer.
    ...
    # Scientific acceptance is signed separately from numerical production.
    # Rebuilding the tables must not replace the accepted peer-review status.
    names = sorted(path.name for path in OUT.iterdir() if path.is_file() and path.name not in {"manifest.json", "curated_manifest.json"})
    ...
    REPRO_MANIFEST.parent.mkdir(parents=True, exist_ok=True)
    REPRO_MANIFEST.write_text(json.dumps(manifest, ensure_ascii=False, indent=2, allow_nan=False)+"\n",
                              encoding="utf-8", newline="\n")
    return {"main_Loss": main_prediction["Loss"], "pair_count": pair_count,
            "B4_B5": b45, "B9_B10": b910,
            "manifest_sha256": sha256(REPRO_MANIFEST)}
