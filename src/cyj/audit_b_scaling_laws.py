...
DEFAULT_OUTPUT = ROOT / "data/processed/Q2/b_data_audit.json"
...
def validate_input_version(
    ...
    """Verify immutable raw manifests from files present in this main tree.

    The historical producer required a Git object from its member branch.
    Main's reproducible release pins the same data by SHA256 instead.
    """
    if input_version != "main-frozen-raw-v1":
        raise ValueError("input version must be main-frozen-raw-v1")
    expected = (
        (manifest, "3377a36c5f6fcabb79abca5fe5621047a3abd9be6dffd9491bc8c651b19f903f"),
        (source_manifest, "34e81dabf46302eebaac8551fa18d2e72acd0833b1cf0276690bee897f1323db"),
    )
    for path, digest in expected:
        if not path.is_file() or sha256(path) != digest:
            raise ValueError(f"raw source manifest identity mismatch: {path}")
