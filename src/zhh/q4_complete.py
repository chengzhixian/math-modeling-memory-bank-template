...
def q3_sensitivity(bridge):
    ref=git('rev-parse',Q3_REF)
    rel='outputs/chm/q3_conditional_v8/'
    for name in ['manifest.json','fixed_policy_grid.csv']:
        raw=subprocess.check_output([GIT,'-C',str(ROOT),'show',f'{ref}:{rel}{name}'])
    ...
    return {'branch':'integration/chm-q1-clean-20260923','commit':ref,'manifest_hash':sha(OUT/'upstream_q3_manifest.json'),
        ...
        'scope':'Unintegrated Q3 v8 consumed only as explicit coordinate-equality sensitivity, not an empirical benchmark optimization result.'}


def c8_profiles():
    base=ROOT/'outputs/zhh/c8_bbh_task_aggregation.csv'
    r=pd.read_csv(base)
...
def main():
    ...
    canonical=ROOT/'outputs/zhh'
    (canonical/'q4_results.json').write_text((OUT/'results.json').read_text(encoding='utf-8'),encoding='utf-8',newline='\n')
    published=pd.read_csv(OUT/'frontier_forecast.csv')
    published[published['filter']=='base_chat_domain_latest'].to_csv(canonical/'frontier_forecast.csv',index=False,lineterminator='\n')
    (canonical/'bridge_interface.json').write_text(json.dumps(json_safe({
        'version':'zhh.bridge.conditional.v2','ready_for_conversion':False,
        'formal_cross_coordinate_status':'unidentified','conditional_mapping_status':'sensitivity_only',
        'target':'equal mean of six leaderboard scores','mappings_file':'outputs/zhh/q4_v2/bridge_mappings.csv',
        'diagnostics_file':'outputs/zhh/q4_v2/results.json','Q3_sensitivity_file':'outputs/zhh/q4_v2/q3_bridge_sensitivity.csv',
        'source_family_holdout':bridge['source_holdout'],'prediction_interval':None,
        'consumer_action':'Formal requests return unidentified; explicit same-coordinate assumption requests may read supported sensitivity rows only.',
        'upstream_Q3':result['q3_consumption']}),ensure_ascii=False,indent=2)+'\n',encoding='utf-8',newline='\n')
    ...
    files+=[ROOT/'outputs/zhh/c8_bbh_task_aggregation.csv',ROOT/'problem/readable/DATA_DESCRIPTION_VISIBLE.md',Path(__file__)]
    dump('manifest.json',{'schema':result['schema'],'seed':SEED,'code_base_commit':git('rev-parse','HEAD'),
        ...
        'canonical_output_sha256':{str(p.relative_to(ROOT)).replace('\\','/'):sha(p) for p in [canonical/'q4_results.json',canonical/'frontier_forecast.csv',canonical/'bridge_interface.json']},
        'upstream':result['q3_consumption'],'output_sha256':{p.name:sha(p) for p in sorted(OUT.iterdir()) if p.is_file() and p.name!='manifest.json'},
