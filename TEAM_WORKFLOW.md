...
集成人将当前有效四问的 **main 发布副本**按问题收录于 `outputs/Q1` 至 `Q4`、`interfaces/Q1` 至 `Q4`、`experiments/Q1` 至 `Q4` 及论文对应分节；成员目录规则继续适用于个人分支的研究与完整生产日志。冻结清单中的原成员路径仅保留来源身份，main 的精简路径由问题目录索引说明。集成以“一问的完整可复现交付包”为单位，必须包含答案、生产代码及必要依赖、核心结果、接口、复现说明和验收证据；缓存与过时发布包不纳入。
...
3. 按依赖先后顺序串行验收，一次处理一个成员的变更。长期个人分支含研究过程与旧版时，从精确验收提交选择性整理到专用交付分支，再向 main 提交；不整体合并个人分支或重写成员历史。交付包须包含当前生产代码、实际依赖、核心输入/结果、接口、复现和验收证据。
...
推荐通过 PR 合并专用交付分支；没有 PR 工具时，集成人核对远端 main 未前进及交付分支全部门禁后，执行仅快进合入并推送，再用 `ls-remote` 核对远端 SHA。无需为备份成员工作而把未完成代码合入 main。
