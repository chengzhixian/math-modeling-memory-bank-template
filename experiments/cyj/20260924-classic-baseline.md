...
- 分支：`team/cyj-scaling`。
- 正式代码/输入提交：`3b9cbff1362349bd9dc9d94d56c409f7d93654be`。
- main 基线：`a0932fd92b3a46cef8eb0bf563df1e6abc9396ef`。
- 输入：`data/raw/real_attachments/B_scaling_laws/B1_neural_scaling_law.csv`；1,176 行，8 个 N 规模组，每组 147 行。
...
- `py_compile` 通过；`unittest` 13/13 通过。
...
- 三种检查的 RMSE 均小于预设诊断阈值 0.001，`near_exact_reconstruction_triggered=true`。这可能来自共同的确定性构造或强预处理，不能据此证明独立真实泛化。
