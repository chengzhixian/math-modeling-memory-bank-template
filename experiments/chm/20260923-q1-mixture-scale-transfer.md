...
固定效应模型解释域内 (\log b) 变化的 (R^2approx0.652)。

但域特异 $\eta_k$ 范围为 0.0447–0.3074，说明不同目标域的配比效应衰减速度差异不可忽略。因此下游至少需要“公共 eta 主模型 + 域特异 eta 敏感性”两套结果。
...
其中 (c_k) 用本实验拟合的目标域幅度常数，(eta) 主值用 0.14537，敏感性使用 bootstrap 区间和 $\eta_k$。
