...
本地复跑发现四个 alpha 改变：wikipedia_en 0.01→0.1，stackexchange 0.1→0.01，gutenberg_pg_19 0.001→0.1，pile_cc 0.001→0.01。完整数值差异见 outputs/chm/q1_local_reproduction_check.json；CV 对照见 outputs/chm/local_recheck_v1/web_local_cv_comparison.csv。

旧输出已移至 outputs/chm/archive/web_v0/ 供追溯，本次可复现配比交付统一放在 outputs/chm/local_recheck_v1/。其中文件名沿用脚本的 v0 名称，但目录是新版本边界；不得将旧系数与新尺度校准混用。
