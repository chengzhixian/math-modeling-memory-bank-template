...
`model_variant="ridge_main"` remains the only callable v6 variant. A later independent Q1 upstream bundle now supplies complete 13-target second-order coefficients for the **offline** Q2 analysis in `src/cyj/analyze_domain_interactions_q2.py`; requesting `interaction_sensitivity` through v6 still fails explicitly because the Q3 consumption contract remains Ridge and this Q1 interaction candidate awaits CHM owner signoff. The v6 behavior and existing fixtures are unchanged.
