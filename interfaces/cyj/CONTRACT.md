...
B9/B10 外推讨论尚待完成。已有分组验证、bootstrap 和消融均不能绕过这些门槛。chm 可消费接口做联调和标记清楚的情景分析，正式 Q3 配置须待后续 validated predictor；zhh 必须传播独立的 Loss–Benchmark 桥接误差。机器可读使用策略在质量审计 JSON 的 `usage_policy`，预测器 schema 和数值未变，测试现为 29 项通过。
