...
问题一正式发布版本为 \texttt{chm.q1.v2.0}，其清单 SHA256 为
\texttt{c621f7e405106e42720f918f490235b5e8becefb0f7c8cb997736e96337117a9}。
...
这些数值指向 \texttt{outputs/cyj/q2\_v7/policy\_solutions.csv}、
\texttt{policy\_details.json} 和 CHM 的 \texttt{q1\_v2\_hull\_bounds/bounds.json}，
不是联合 A/B 实测改进。
...
数值与方向可行性见 \texttt{domain\_transfer\_effects.csv}、
\texttt{domain\_complementarity.csv}；在观测配方集上的连续方向只属于其凸包，不等于新的已观测配方。
...
固定政策文件、输入哈希、代码、环境与输出清单在
\texttt{outputs/cyj/q2\_v7/}；Q2 运行期间的 Python 文件打开审计记录原始 A 为零次，
其覆盖范围与限制由 \texttt{outputs/cyj/q2\_v7\_runtime\_audit.json} 说明。
