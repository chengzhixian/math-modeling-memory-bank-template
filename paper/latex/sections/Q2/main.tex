...
采用公开标度研究的 $N,D$ 幂律形式作为候选函数族\cite{hoffmann2022}，但具体系数仅由附件 B1 的 Pythia 训练轨迹估计\cite{pythia2023}。以 B1 为唯一经典骨架来源，在 $N\in[0.070542,11.965825]$、
...
\end{equation}
式中基础情景 $q=g(Q_A(\boldsymbol p))$ 随配比锁定；固定 $\boldsymbol p$
时，它不是第四个独立决策自由度。另设可支付额外数据处理成本的情景，
才把 B7 的原生 $Q_B$ 作为独立控制量，并固定配比求解
$L_B(N,D,Q_B)\exp\{r_w(\boldsymbol p)\}$。
两种机制的质量坐标和操作方式必须分别报告，第三问将其并列配置。
...
问题三现已发布固定配比、87 配方联立和原生 $Q_B$ 的 36 格条件结果，
CHM 的独立网格审计在三种模式均无不一致项；
\texttt{outputs/Q2/acceptance.json} 中的 pending/false 是第二问生产者发布时
尚未完成下游验收的历史快照，现时集成状态由
\texttt{outputs/Q3/ANSWER.md} 和独立审计表记录。它不改变本节的数值身份。
...
例如 $N=1$ B 时，代理质量增加 $0.10$ 的模型内部改善等价于
旧质量下扩模至 $1.317062$ B；增加 $0.05$ 则等价于扩模至
$1.144490$ B。若提高质量 $0.05$ 并保持 Loss 不变，
$N$ 可降至 $0.875789$ B。固定配比却改变由它派生的代理质量，
这里只是独立质量投入情景的模型内替代运算，不是已观测的配比干预。
...
$0.075078$。这些方向描述单域替换，方向曲率的正负不能直接命名为
两域互补，也没有因果含义。
...
为给“互补/替代”一个可操作判据，固定参考配比为 512 条 A4 配方均值、
$N=1$ B、$D=100$ B、13 目标等权，固定补偿域 $k=\texttt{pile\_cc}$。
分别把 $\delta$ 比例从 $k$ 转给域 $i$、域 $j$，再同时转给两域；
四个配比均经 A4 凸包检查。定义有限联合效应
\[
I_{ij\mid k}(\delta)=L(\boldsymbol p+\delta(\boldsymbol e_i+\boldsymbol e_j-2\boldsymbol e_k))
-L(\boldsymbol p+\delta(\boldsymbol e_i-\boldsymbol e_k))
-L(\boldsymbol p+\delta(\boldsymbol e_j-\boldsymbol e_k))+L(\boldsymbol p).
\]
在固定补偿规则与“Loss 越低越好”下，$I<0$ 表示联合投入比两个
单独投入的改善之和更大，称\emph{操作性互补}；$I>0$ 称操作性替代。
这一定义包含共同移出 $k$ 的非线性，不能隔离两域的纯因果交互。
\begin{table}[htbp]\centering\small
\caption{均值配比、$\delta=0.005$、固定从 pile\_cc 补偿的有限联合效应（Loss）}
\label{tab:cyj-q2-joint}
\begin{tabular}{lrrrl}\toprule
同时增加的域 & 单独 $i$ 变化 & 单独 $j$ 变化 & $I_{ij\mid k}$ & 判断\\\midrule
gutenberg\_pg\_19、europarl & $-0.001662$ & $-0.001729$ & $-0.00001844$ & 互补\\
arxiv、freelaw & $-0.001973$ & $+0.000235$ & $+0.00001172$ & 替代\\
arxiv、github & $-0.001973$ & $+0.000656$ & $-0.00000244$ & 弱互补\\
\bottomrule\end{tabular}
\end{table}
第一行两个单独投入都改善 Loss，联合再多改善约 $1.84\times10^{-5}$；
第三行虽有负联合效应，单独提高 github 仍使 Loss 变差，
故“互补”并不自动构成提高其配比的建议。
第一行在 $\delta=0.002,0.005,0.01$、质量映射斜率
$0.5/1/1.5$ 倍及配比桥强度 $0/1/2$ 的所测情景中保持负号；
arxiv--freelaw 在强度为 0 时转为负号，故其替代判断依赖桥接假设。
补算代码及逐格结果为\texttt{audits/review\_followup\_20260926/evidence.py}
与\texttt{computed\_evidence.json}；原
\texttt{domain\_pair\_interaction.csv} 的统一
\texttt{interaction\_label} 只是旧版警示字符串，不用作本表分类。

\subsection{外推分析、适用范围与模型局限}

B7 为半合成网格，B3 为 B1 风格插值，B4/B5 的支持内同族下降方向
分别为 12/12 与 7/7；这支持规模方向一致性，却对任意单调下降模型
区分力弱，也不检验跨族绝对 Loss。B10 的 128 条 Loss 为 estimated，
与审计时 B1 曲线的描述性 RMSE 为 0.00110477、MAE 为 0.00063013，
说明两张\emph{估算构造}在这些坐标上数值接近，不能据此承诺真实外推精度。
其中 B10 的 $N$ 全部超过 B1 上界，$D$ 有 $79.6875\%$ 超过上界；
5 组重复 $(N,D)$ 坐标还给出相同估算 Loss，增加了非独立性顾虑。
$Q_A\to Q_B$、A 侧逐目标相对 Loss 到 B 侧
...
给出 $N,D,Q,\boldsymbol p$ 的条件局部效用、质量增加 0.10 等价于
旧质量下 $N:1\to1.317062$ B 的示例替代、136 对领域可行局部方向，
并用固定补偿的有限联合效应区分代表性互补与替代。
跨来源幅度仍待成对实验标定；问题三的条件消费者验收已在主分支完成。
