...
\end{equation}
该归一化只用于可视化，不进入任何预测或优化计算。Data Mixing Laws 的 Fig.~4 也采用按验证域最大绝对系数归一化的方式展示训练域与验证域之间的作用模式；本文的 $\widetilde{\boldsymbol B}$ 与其作用矩阵思想相同，但数值来自本题 A4+A5 的 Ridge，而不是该文的指数混合律。
