...
用户已明确批准将题面和全部原始附件上传到现有公开仓库 chengzhixian/math-modeling-memory-bank-template，并要求先将角色更名为 chm、cyj、zhh。更名和路径更新已完成，原始资料 2,014 文件的 SHA256 与 LFS fsck 已通过；本次执行上传，结果以 Git 远端与 LFS 核验为准。
